...
<meta name="keywords" content=",8558,8558游戏,8558游戏平台">
...
<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq(50,5,2,1);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
...
<?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq('select classid,classname,classpath from [!db.pre!]enewsclass where bclassid='.$GLOBALS[navclassid].' and islast=1 order by myorder',0,24,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
...
<div class="content">
<div class="titles">
<div class="titles_left"><?=$bqr[classname]?></div>
<div class="titles_right"><a href="<?=$public_r[newsurl]?><?=$bqr[classpath]?>/">更多&gt;&gt;</a></div>
</div>
<ul>

<?php
$sql=$empire->query("select title,titleurl,newstime,titlepic from {$dbtbpre}ecms_news where classid='{$bqr[classid]}' order by newstime desc limit 5");
while($info=$empire->fetch($sql))
{
?>
<li><a href="<?=$info[titleurl]?>" title="<?=$info[title]?>" target="_blank"><img src="<?=$info[titlepic]?>"  alt="<?=$info[title]?>"><p><?=sub($info[title],0,20,false)?></p></a></li>
